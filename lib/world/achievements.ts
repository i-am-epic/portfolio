...
function load(): Persisted {
  if (typeof window === "undefined") return { unlocked: {}, openedChests: [], boardsRead: [], bestRunMs: null }
  try {
  ...
  return { unlocked: {}, openedChests: [], boardsRead: [], bestRunMs: null }
}
...
export const useAchievements = create<AchState>((set, get) => {
  const persist = () => {
    ...
    const { unlocked, openedChests, boardsRead, bestRunMs } = get()
    try {
      window.localStorage.setItem(STORE_KEY, JSON.stringify({ unlocked, openedChests, boardsRead, bestRunMs }))
    } catch {}
  }
